...
- sous_type_parcelle:
  ...
  En pratique, l'amont fournit des contours d'**unites de gestion**
  (UGF), chacune portant sa reference cadastrale. Ce sont des limites de
  gestion, et `"section"` leur convient. Consequence a connaitre : une
  unite taillee dans une PORTION de parcelle a des cotes de decoupe
  interne, que rien ne materialise au sol – un lineaire qui les suit
  ressort en `layon_parcellaire` sans en etre un. `CLASSE_MOTIF` porte
  alors `parcelle`, ce qui permet de les retrouver et de trancher au cas
  par cas.
